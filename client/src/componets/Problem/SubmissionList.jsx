      ...
      {loading ? (
        ...
        <div className="mt-4 overflow-x-auto">
          {submissions.length > 0 ? (
            <table className="min-w-full text-gray-200 border-separate" style={{ borderSpacing: "0 15px" }}>
              <thead>
                    ...
                    <td className="px-4 py-2 font-semibold text-center capitalize">
                      <span
                        className={`px-2 py-1 rounded ${
                          submission.status === "completed" ? "bg-green-600 text-green-200" : "bg-red-600 text-red-200"
                        }`}
                      >
                        {submission.status === "completed" ? "Accepted" : "Rejected"}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-center capitalize">{submission.language}</td>
                    <td className="px-4 py-2 text-center">
                      {submission.execution_time ? `${submission.execution_time.toFixed(2)} ms` : "N/A"}
                    </td>
                    <td className="px-4 py-2 text-center">
                      {submission.memory_usage ? `${submission.memory_usage.toFixed(2)} MB` : "N/A"}
                    </td>
            ...
            <p className="text-gray-400 text-center mt-4">No submissions found.</p>
          )}
        </div>
      )}
    </div>
