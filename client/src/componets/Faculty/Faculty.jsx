...
const Faculty = () => {
    ...
    const handleAcceptAll = async () => {
        ...
        try {
            const response = await axiosInstance.post(`/faculty/accept-all-requests`, {
                facultyId,
            });

            if (response.data.success) {
                toast.success(response.data.message || "All pending users have been approved.");
                setPendingUsers([]);
                fetchUsers(currentPage);
            } else {
    ...
    const handleDeclineAll = async () => {
        setActionLoading(true);
        try {
            const response = await axiosInstance.post(`/faculty/decline-all-requests`, {
                facultyId: user._id
            });
        ...
        <>
            <div className="bg-gray-900 text-gray-100 min-h-screen overflow-hidden">
                <div className="flex justify-between items-center mb-4 p-6 pt-[100px]">
                    <h1 className="text-2xl font-bold">Pending User Requests For Subject {user?.subject}</h1>
                ...
                {loading ? (
                    ...
                    <div className="p-4">
                        <table
                            className="min-w-full  text-lg text-left text-gray-500"
                        >
