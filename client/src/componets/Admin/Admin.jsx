...
const Admin = () => {
    ...
    const fetchUsers = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axiosInstance.get(`/admin/get-pending-users?page=${page}`);
    ...
    const handleAcceptUser = async (userId) => {
        ...
        try {
            const response = await axiosInstance.post(`/admin/accept-request`, { userId });
            if (response.data.success) {
    ...
    const handleDeclineUser = async (userId) => {
        setActionLoading(userId);
        try {
            const response = await axiosInstance.post(`/admin/decline-request`, { userId });
            if (response.data.success) {
            ...
            <div className="bg-gray-900 text-gray-100 min-h-screen">
                <div className="flex justify-between items-center mb-4 p-6 pt-[100px]">
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            {actionLoading ? <BeatLoader size={10} color="#fff" /> : "Accept All"}
                        </button>
                        <button
                        ...
                        >
                            {actionLoading ? <BeatLoader size={10} color="#fff" /> : "Decline All"}
                        </button>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <table className="min-w-full table-auto text-lg text-left text-gray-100">
                        ...
                        <tbody>
                            {pendingUsers.length > 0 ? (
                                pendingUsers.map((user, index) => (
                                    <tr key={user._id} className={`${index % 2 === 0 ? "bg-gray-800" : "bg-gray-700"} hover:bg-gray-600`}>
                                        <td className="py-3 px-6">{(currentPage - 1) * PAGE_SIZE + index + 1}</td>
                                        ...
                                        <td className="py-3 px-6">{user.semester}</td>
                                        ...
                                        <td className="py-3 px-6 flex gap-2">
                                            ...
                                            >
                                                {actionLoading === user._id ? <BeatLoader size={10} color="#fff" /> : "Accept"}
                                            </button>
                                            <button
                                            ...
                                            >
                                                {actionLoading === user._id ? <BeatLoader size={10} color="#fff" /> : "Decline"}
                                            </button>
                                        </td>
                                    </tr>
                                ))
                                ...
                                <tr>
                                    <td colSpan="8" className="py-3 px-6 text-center text-gray-300">No pending users found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
